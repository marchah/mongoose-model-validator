...
function requiredValidator(Model, options, analyzed) {
  describe('Required #', () => {
    if (options.schema.required && _.isArray(options.schema.required)) {
      it('should have ' + analyzed.required.ALL.length + ' required fields', () => {
        expect(options.schema.required).to.eql(analyzed.required.ALL);
      });
      /*it('should require ' + options.schema.required.toString(), () => {
...
console.log(validationError);
    ...
    else if (options.schema.required) {
...
function uniqueValidator(Model, options, analyzed) {
  describe('Unique #', () => {
    const keys = options.schema.unique;
    const unchecked = _.difference(analyzed.unique.ALL, keys);
    if (options.schema.unique && _.isArray(options.schema.unique)) {
...
function defaultValidator(Model, options, analyzed) {
  describe('Default #', () => {
    const keys = _.keys(options.schema.default);
    const unchecked = _.difference(analyzed.default.ALL, keys);
    if (options.schema.default && _.isPlainObject(options.schema.default)) {
      for (let i = 0; i !== keys.length; i++) {
        it('should validate default value for `' + keys[i] + '`', (done) => {
          ...
          else if (_.isFunction(options.schema.default[keys[i]]) || _.isFunction(analyzed.schema[keys[i]].default)) {
            done('Functions are not supported');
          }
          else {
            expect(options.schema.default[keys[i]]).to.eql(analyzed.schema[keys[i]].default);
            done();
          }
        });
      }
    }
    else if (options.schema.default) {
...
function enumValidator(Model, options, analyzed) {
  describe('Enum #', () => {
    const keys = _.keys(options.schema.enum);
    const unchecked = _.difference(analyzed.enum.ALL, keys);
    if (options.schema.enum && _.isPlainObject(options.schema.enum)) {
      for (let i = 0; i !== keys.length; i++) {
        it('should validate possible values for `' + keys[i] + '`', (done) => {
          ...
          else if (!_.isArray(options.schema.enum[keys[i]]) || !_.isArray(analyzed.schema[keys[i]].enum)) {
            done('Only array are supported');
          }
          else {
            expect(options.schema.enum[keys[i]]).to.eql(analyzed.schema[keys[i]].enum);
            done();
          }
        });
      }
    }
    else if (options.schema.enum) {
...
function minValidator(Model, options, analyzed) {
  describe('Min #', () => {
    const keys = _.keys(options.schema.min);
    const unchecked = _.difference(analyzed.min.ALL, keys);
    if (options.schema.min && _.isPlainObject(options.schema.min)) {
      for (let i = 0; i !== keys.length; i++) {
        it('should validate min value for `' + keys[i] + '`', (done) => {
          if (_.isUndefined(analyzed.schema[keys[i]].min)) {
            done('doesn\'t have min value');
          }
          else if (!_.isNumber(options.schema.min[keys[i]]) || !_.isNumber(analyzed.schema[keys[i]].min)) {
            done('Only number are supported');
          }
          else {
            expect(options.schema.min[keys[i]]).to.eql(analyzed.schema[keys[i]].min);
            done();
          }
        });
      }
    }
    else if (options.schema.min) {
...
function maxValidator(Model, options, analyzed) {
  describe('Max #', () => {
    const keys = _.keys(options.schema.max);
    const unchecked = _.difference(analyzed.max.ALL, keys);
    if (options.schema.max && _.isPlainObject(options.schema.max)) {
      for (let i = 0; i !== keys.length; i++) {
        it('should validate max value for `' + keys[i] + '`', (done) => {
          if (_.isUndefined(analyzed.schema[keys[i]].max)) {
            done('doesn\'t have max value');
          }
          else if (!_.isNumber(options.schema.max[keys[i]]) || !_.isNumber(analyzed.schema[keys[i]].max)) {
            done('Only number are supported');
          }
          else {
            expect(options.schema.max[keys[i]]).to.eql(analyzed.schema[keys[i]].max);
            done();
          }
        });
      }
    }
    else if (options.schema.max) {
...
function minlengthValidator(Model, options, analyzed) {
  describe('Minlength #', () => {
    const keys = _.keys(options.schema.minlength);
    const unchecked = _.difference(analyzed.minlength.ALL, keys);
    if (options.schema.minlength && _.isPlainObject(options.schema.minlength)) {
      for (let i = 0; i !== keys.length; i++) {
        it('should validate minlength value for `' + keys[i] + '`', (done) => {
          ...
          else if (!_.isNumber(options.schema.minlength[keys[i]]) || !_.isNumber(analyzed.schema[keys[i]].minlength)) {
            done('Only number are supported');
          }
          else {
            expect(options.schema.minlength[keys[i]]).to.eql(analyzed.schema[keys[i]].minlength);
            done();
          }
        });
      }
    }
    else if (options.schema.minlength) {
...
function maxlengthValidator(Model, options, analyzed) {
  describe('Maxlength #', () => {
    const keys = _.keys(options.schema.maxlength);
    const unchecked = _.difference(analyzed.maxlength.ALL, keys);
    if (options.schema.maxlength && _.isPlainObject(options.schema.maxlength)) {
      for (let i = 0; i !== keys.length; i++) {
        it('should validate maxlength value for `' + keys[i] + '`', (done) => {
          ...
          else if (!_.isNumber(options.schema.maxlength[keys[i]]) || !_.isNumber(analyzed.schema[keys[i]].maxlength)) {
            done('Only number are supported');
          }
          else {
            expect(options.schema.maxlength[keys[i]]).to.eql(analyzed.schema[keys[i]].maxlength);
            done();
          }
        });
      }
    }
    else if (options.schema.maxlength) {
...
function customValidator(Model, options, analyzed) {
  describe('Custom #', () => {
    if (options.schema.custom && _.isPlainObject(options.schema.custom)) {
      const keys = _.keys(options.schema.custom);
      for (let i = 0; i !== keys.length; i++) {
        it(keys[i], options.schema.custom[keys[i]]);
      }
    }
    else if (options.schema.custom) {
