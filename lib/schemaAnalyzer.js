...
function fieldSchemaAnalyzer(fieldSchema, path, analyzed, pluginFields) {
  ...
  if (fieldSchema.instance === 'Mixed' && !_.isUndefined(fieldSchema.options) && !_.isUndefined(fieldSchema.options.type)) {
    ...
    keys.forEach((key) => {
      if (_.indexOf(constants.IGNORED_FIELD.ALL, key) === -1  && _.indexOf(pluginFields, path + '.' + key) === -1) {
...
function schemaAnalyzer(Model, pluginFields) {
  const analyzed = {
    schema: {},
    required: {
      ALL: [],
  ...
  keys.forEach((key) => {
    if (_.indexOf(constants.IGNORED_FIELD.ALL, key) === -1 && _.indexOf(pluginFields, key) === -1) {
      fieldSchemaAnalyzer(schema[key], key, analyzed, pluginFields);
    }
  });
  return analyzed;
}
