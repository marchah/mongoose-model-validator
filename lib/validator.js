...
function validateVirtualFields(Model, options, VirtualFields) {
  describe('Virtual Fields ->', () => {
    const keys = options.virtualFields;
    const virtualFieldKeys = _.difference(constants.IGNORED_VIRTUAL_FIELD.ALL, _.keys(VirtualFields));
    ...
    if (keys && _.isArray(keys)) {
      for (let i = 0; i !== keys.length; i++) {
        it('should validate virtual field for `' + keys[i] + '`', (done) => {
          ...
          else if (!_.isString(keys[i])) {
            done('Only string are supported');
          }
          else {
      ...
      it('virtualFields should be an array', (done) => {
        done('virtualFields should be an array');
      });
    }
