...
function validateSchema(Model, options, analyzed) {
  describe('Schema ->', () => {
    if (options && options.schema) {
      const schemaValidatorKeys = _.keys(options.schema);
      const unknownKeys = _.difference(schemaValidatorKeys, constants.VALIDATOR.SCHEMA.ALL);
...
function validateEvents(Model, options) {
  describe('Events ->', () => {
    const keys = _.keys(options.events);
    if (keys && _.isPlainObject(options.events)) {
      for (let i = 0; i !== keys.length; i++) {
        if (options.events[keys[i]] === null) {
          it('should validate event `' + keys[i] + '`');
        }
        else {
          it('should validate event `' + keys[i] + '`', (done) => {
            if (!_.isFunction(options.events[keys[i]])) return done('`' + keys[i] + '` should be a function');

            if ((options.events[keys[i]]).length === 0) {
              options.events[keys[i]]();
              done();
            }
            else {
              options.events[keys[i]](done);
            }
          });
        }
      }
