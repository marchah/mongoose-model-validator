        ...
        it('should validate virtual field `' + keys[i] + '`', (done) => {
          if (_.isUndefined(VirtualFields[keys[i]])) {
            ...
            expect(VirtualFields[keys[i]].path).to.exist
              .and.eql(keys[i]);
            if ((toValidate.virtualFields[keys[i]]).length === 0) {
              toValidate.virtualFields[keys[i]]();
              done();
            }
            else {
              toValidate.virtualFields[keys[i]](done);
            }
          }
        });
      }
      ...
      for (let i = 0; i !== keys.length; i++) {
        it('should validate method `' + keys[i] + '`', (done) => {
          if (_.isUndefined(Methods[keys[i]])) {
            ...
            if ((toValidate.methods[keys[i]]).length === 0) {
              toValidate.methods[keys[i]]();
              done();
            }
            else {
              toValidate.methods[keys[i]](done);
            }
          }
        });
      }
      ...
      for (let i = 0; i !== keys.length; i++) {
        it('should validate static `' + keys[i] + '`', (done) => {
          if (_.isUndefined(Statics[keys[i]])) {
            ...
            if ((toValidate.statics[keys[i]]).length === 0) {
              toValidate.statics[keys[i]]();
              done();
            }
            else {
              toValidate.statics[keys[i]](done);
            }
          }
      ...
      for (let i = 0; i !== keys.length; i++) {
        if (toValidate.events[keys[i]] === null) {
          ...
          it('should validate event `' + keys[i] + '`', (done) => {
            ...
            if ((toValidate.events[keys[i]]).length === 0) {
              toValidate.events[keys[i]]();
              done();
            }
            else {
              toValidate.events[keys[i]](done);
            }
          });
        }
      }
